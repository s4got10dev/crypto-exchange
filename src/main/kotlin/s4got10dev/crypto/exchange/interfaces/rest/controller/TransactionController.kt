...
class TransactionController(
  ...
  override suspend fun getTransactions(
    ...
    val transactions = transactionAdapter.transactionsQuery(authPrincipal?.userId, page, size)
      .let { transactionService.getTransactions(it) }
    return ResponseEntity.ok(transactions.map { transaction -> TransactionResponse.from(transaction) })
  }
}
